...
import { ApiGetCall } from "../../api/ApiCall"; // Using non-paginated API call
...
export const CippFixedDataTable = (props) => {
  const {
    queryKey,
    data = [],
    columns = [],
  ...
  // Use the fixed (non-paginated) API call
  ...
  // Since there's no pagination, we simply extract the records once the API call succeeds.
  useEffect(() => {
    if (getRequestData.isSuccess) {
      ...
      if (api.dataKey) {
        ...
        records = getNestedValue(getRequestData.data, api.dataKey) || [];
      } else {
        records = getRequestData.data;
      }
      setUsedData(records);
    }
  ...
  useEffect(() => {
    ...
    if (columns.length === 0 && configuredSimpleColumns.length === 0) {
      ...
      finalColumns = [
        ...columns,
        ...apiColumns.filter((col) => !providedColumnKeys.has(col.id))
      ];
  ...
  const table = useMaterialReactTable({
    ...
    renderEmptyRowsFallback: ({ table }) =>
      getRequestData.data?.Metadata?.QueueMessage ? (
        <Box sx={{ py: 4 }}>
          <center>
            <Info /> {getRequestData.data?.Metadata?.QueueMessage}
          </center>
        </Box>
      ) : undefined,
          ...
          <CardContent sx={{ padding: "1rem" }}>
            <Scrollbar>
              {!Array.isArray(usedData) && usedData ? (
                ...
                <>
                  {(getRequestData.isSuccess || (data && !getRequestData.isError)) && (
                    <MaterialReactTable
